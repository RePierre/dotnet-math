...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgef4379a},fragile]{Beneficiile \texttt{TPL}\footnote{\url{https://docs.microsoft.com/en-us/dotnet/standard/parallel-programming/task-parallel-library-tpl}}}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb3d7b22}]{Imaginea de ansamblu\footnote{\url{http://www.albahari.com/threading/part5.aspx}}}
\begin{center}
\includegraphics[height=.7\textheight]{img/tpl.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org0231294},fragile]{Ce este un \texttt{Task}}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgaa482a6},fragile]{\texttt{Task} vs \texttt{Thread}}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org65e0923},fragile]{\texttt{Task} vs \texttt{Thread}\footnote{Diagrama este simplificată la un singur fir de execuţie per nucleu (core) .}}
 \begin{center}
\includegraphics[height=.8\textheight]{img/task-parallelism.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org58f1773},fragile]{Async \& await}
 \begin{itemize}
...
\label{sec:org371bfc0}
\begin{frame}[label={sec:orgb62db70},fragile]{Ce este \texttt{middleware}?}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga670307},fragile]{\texttt{Middleware}\footnote{\url{https://andrewlock.net/asp-net-core-in-action-what-is-middleware/}}}
 \begin{center}
\includegraphics[width=\textwidth]{img/middleware.png}
\end{center}
\end{frame}
\section{Securitatea în \texttt{ASP.NET Core MVC}}
\label{sec:org469d55d}
\begin{frame}[label={sec:org940481b}]{Autentificare}
\begin{block}{Autentificare}
\vskip 0.1in
Procesul de preluare a datelor care atestă identitatea unui utilizator şi verificarea acestora.
\end{block}
\end{frame}
\begin{frame}[label={sec:org2356e4d}]{Autorizare}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0687aec}]{ASP.NET Core Identity}
\begin{itemize}
\item Bibliotecă pentru implementarea autentificării.
\item Gestionează utilizatori, parole, roluri ş.a.
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8c46572}]{Componente\footnote{\url{https://docs.microsoft.com/en-us/aspnet/core/security/authentication/identity-custom-storage-providers}}}
\begin{center}
